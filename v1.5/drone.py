...
class Drone():
    def __init__(self, ip: str = '192.168.4.1', mavlink_port: str = '8001', name: str = '') -> None:
        ...
        self.controls: Drone.__Controls = self.__Controls(self.pioneer)
    ...
    class __Controls:
        def __init__(self, drone: Pioneer) -> None:
            ...
            self.__points: List[Dict[str, float]] = []
        ...
        def add_point(self, *args: Dict[str, float]) -> None:
            for point in args:
        ...
        def __get_next_point(self) -> Dict[str, float]:
            next_idx = (self.current_point_idx+1) % len(self.__points)
            return self.__points[next_idx]

        def __calculate_angle_to_point(self, point: Dict[str, float]) -> float:
